...
try {
  ...
  const initialUi = await evaluate(`({
    ...
    customChecked: document.querySelector('[data-custom-sampling]')?.checked,
    homeButton: document.querySelector('#return-to-unit-chooser')?.textContent,
  ...
  await evaluate(`document.querySelector('[data-custom-sampling]').click()`);
  await waitFor(evaluate, `localStorage.getItem('autodrill.qa.samplingMode')==='custom'&&document.querySelector('[data-custom-sampling]').checked`, 'custom sampling toggle');
  await waitFor(evaluate, `document.querySelector('.print-prefetch-frame[data-prefetch-ready="true"]') !== null`, 'custom next problem prefetched', 20_000);
  ...
  const customAttempt = qa.repository.activeAttempt();
  const customDetail = qa.repository.attemptDetail(customAttempt.id);
  if (customDetail.selection.selection_policy !== 'autodrill_unit_custom_v1'
    || customDetail.selection.model_name !== 'operation_vector_information'
    || customDetail.selection.model_version !== '1'
    || customDetail.selection.selection_probability !== null
    || !customDetail.selection.candidate_scores_json
    || customDetail.original_source_payload.qa_sampling?.mode !== 'custom'
    || !Array.isArray(customDetail.original_source_payload.qa_sampling?.operation_vector_basis)
    || !Array.isArray(customDetail.original_source_payload.qa_sampling?.operation_vector)) {
    throw new Error(`Custom sampling provenance is broken: ${JSON.stringify(customDetail.selection)}`);
  }
  ...
  await evaluate(`window.confirm=()=>true; document.querySelector('#return-to-unit-chooser').click()`);
  await waitFor(evaluate, `document.querySelector('#unit-choice') !== null && !document.querySelector('#return-to-unit-chooser')`, 'return to unit chooser');
  if (qa.repository.activeAttempt() !== null) throw new Error('Returning to the unit chooser left an active attempt behind.');
