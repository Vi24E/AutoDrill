...
test('desktop window uses an ephemeral port and owns server/profile lifecycle', async () => {
  ...
  const browserLauncher = ({ url }) => {
    observedUrl = url;
    const check = `const response=await fetch(${JSON.stringify(`${url}/api/state`)}); if(!response.ok) process.exit(2); const state=await response.json(); if(state.metadata.appVersion!=='0.3.0') process.exit(3);`;
