...
alpha 1.2では、alpha 1.1以降に追加した多数のテーマとMathLiveによる構造化数式入力を正式な現行構成として扱います。実装済みテーマは11個です。
...
小学1年生〜中学3年生の学年選択、`おすすめ`、5段階難易度、Seed再現、単元別URLを備えています。開発中placeholderとして`Dummy1`が各学年の末尾に残っています（実装済みジャンルがある学年にも併存します）。Dummyは生成・印刷を無効化し、単元URLやsitemapには公開されません。
