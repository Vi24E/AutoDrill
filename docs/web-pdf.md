...
`apply_editor_action`, and `grade_answer` with versioned JSON DTO strings. The
...
`src/domain/layout.ts` owns A4 geometry and the 2-column × 10-row cell model.
The q2 paper converts the same top-origin cell positions to page-relative CSS
percentages, while the PDF renderer converts them to pdf-lib's bottom-origin
coordinates. Thus order, margins, row heights, and the central divider have one
serializable source of truth. The PDF renderer derives both pages from that model. The paper uses only the
central column divider, visible problem numbers, expressions, and rectangular
answer boxes; the unit title remains in the q2 ribbon rather than being repeated
on the white sheet. Page one has empty answer boxes; page two contains answers
and has a 180° page rotation. Bytes are
