...
Worksheetはcoreではdecoded `identity`、registry由来のskill/curriculum/layout、registry所定個数のProblemだけを保持するvalidated aggregateである。`problem_set_id`はcore内でidentityから純粋に導出できるが、現行WebにconsumerがないためWorksheet wireへ重複投影しない。Worksheet wireは`schema_version`、`identity`、`layout`、`problems`だけを公開し、skill/curriculumはgenerated theme contract側をauthorityとする。`Worksheet::generated`はidentityとregistrationのschema/theme/revision、problem数、各Problemのschema/themeを一度に検証する。coreの`Problem`はversion、通し番号、numeric theme ID、型付きprompt、validated answer schema、validated canonical answer、そこからregistration contractで導出した`input_interface`、promptとcanonical answerからcore自身が導出したopaque `worked_solution`、そして排他的な`EffortModel`を持つ。generatorはworked-solution fieldを直接組み立てて`Problem`へ注入できない。Column multiplication / long divisionのpartial/step dataはcore domainのprivate valueであり、Webへ必要なshapeだけを`wire.rs`のDTOへ投影する。
